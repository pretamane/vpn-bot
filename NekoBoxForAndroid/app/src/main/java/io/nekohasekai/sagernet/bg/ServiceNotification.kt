...
/**
 * User can customize visibility of notification since Android 8. The default visibility:
 *
 * Android 8.x: always visible due to system limitations VPN: always invisible because of VPN
 * notification/icon Other: always visible
 *
 * See also:
 * https://github.com/aosp-mirror/platform_frameworks_base/commit/070d142993403cc2c42eca808ff3fafcee220ac4
 */
class ServiceNotification(
    ...
    companion object {
        ...
        fun genTitle(ent: ProxyEntity): String {
            val gn =
                    if (DataStore.showGroupInNotification)
    ...
    suspend fun postNotificationTitle(newTitle: String) {
        useBuilder { it.setContentTitle(newTitle) }
        update()
    }
    ...
    suspend fun postNotificationWakeLockStatus(acquired: Boolean) {
        ...
        useBuilder {
            it.priority =
                    if (acquired) NotificationCompat.PRIORITY_HIGH
    ...
    private val builder =
            NotificationCompat.Builder(service as Context, channel)
                    ...
                    .setPriority(
                            if (visible) NotificationCompat.PRIORITY_LOW
    ...
    private suspend fun show() = useBuilder {
        try {
            ...
            Toast.makeText(SagerNet.application, "startForeground: $e", Toast.LENGTH_LONG).show()
        }
    }
